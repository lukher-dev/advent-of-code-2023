...
func findHowManyBricksAreSupported(brick int, data []brick, occupied map[[3]int]int) int {
	thisBricksSupports := make(map[int]bool)
	thisBricksSupports[brick] = true
	for i := 0; i < len(data); i++ {
		if len(data[i].getsSupportedBy) != 0 {
			count := 0
			for j := 0; j < len(data[i].getsSupportedBy); j++ {
				if _, ok := thisBricksSupports[data[i].getsSupportedBy[j]]; ok {
					count += 1
				}
			}
			if count == len(data[i].getsSupportedBy) {
				thisBricksSupports[i] = true
			}
		}
	}

	return len(thisBricksSupports) - 1
}
...
func main() {
	...
	result := 0
	for i := 0; i < len(data); i++ {
		result += findHowManyBricksAreSupported(i, data, occupied)
	}
	fmt.Println(result)
}
